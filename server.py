...
def update_config(autogenerated_effects=None):
    ...
    print(f'finishing up to imports took {time.perf_counter() - begin:.3f} seconds')
    ...
    print(f'finishing up to getting song detail lengths {time.perf_counter() - begin:.3f} seconds')
    ...
    print(f'finishing up to beat reading took {time.perf_counter() - begin:.3f} seconds')

    for effect_name in graph:
        effects_config_sort([effect_name])
    print(f'finishing up the graph sorting took {time.perf_counter() - begin:.3f} seconds')
    ...
    for effect_name, effect in effects_config.items():
        ...
        if 'song_path' in effect and effect['song_path'] in songs_config:
            ...
            if 'loop' not in effect:
                effect['loop'] = True
    print(f'(done with update_config) finishing up to effect defaults took {time.perf_counter() - begin:.3f} seconds')
    ...
    print(f'finishing up to setting all_effects defaults took {time.perf_counter() - begin:.3f} seconds')
    ...
    print(f'finishing up to simple effects took {time.perf_counter() - begin:.3f} seconds')
    ...
    for effect_name in complex_effects:
        ...
        for component in effect['beats']:
            ...
            for i in range(length):
                # 12% of the time
                channels = reference_beats[(i + offset) % reference_length]
    ...
    # block += time.perf_counter() - starter
    # print(f'time spent in block: {block}')


    print(f'finishing up to complex effects took {time.perf_counter() - begin:.3f} seconds')
