effects = {
    "everything bottom glow": {
        "length": 1,
        "beats": [
            [1, "Pink bottom", 1, 0.15, 0.15],
        ],
    },
    "everything melody glow": {
        "length": 8,
        "beats": [
            [1, "Pink bottom", 3.5, 0.15, 0.15],
            [4.5, "Orange bottom", 2, 0.15, 0.15],
            [6.5, "Cyan bottom", 3, 0.15, 0.15],
        ],
    },
    "everything melody 2 glow": {
        "length": 8,
        "beats": [
            [1, "Pink bottom", 3.5, 0.15, 0.15],
            [4.5, "Orange bottom", 2, 0.15, 0.15],
            [6.5, "Cyan bottom", 3, 0.15, 0.15],
        ],
    },
    "everything kick": {
        "length": 1,
        "beats": [
            [1, "White bottom", 0.15, 0.2, 0.05],
        ],
    },
    "everything kick line": {
        "length": 4,
        "beats": [
            [1, "everything kick", 1],
            [2, "everything kick", 1],
            [3, "everything kick", 1],
            [3.5, "everything kick", 1],
            [4, "everything kick", 1],
        ],
    },
    "everything show": {
        "beats": [
            [1, "rainbow good slow top", 120, .1, .6],
            [1, "everything bottom glow", 120],
            [1, "everything kick", 31],
            [33, "everything kick", 24],
            [65, "everything kick line", 56],
            [121, "everything melody glow", 72],
            [121, "UV", 8],
            [129, "rainbow good slow top", 60, .6, .6],
            [129, "everything kick line", 64],
            [189, "UV", 4],
            [193, "rainbow good slow top", 64, .6, 0],
            [193, "everything kick", 16],
            [193, "everything melody 2 glow", 16],
            [209, "everything kick", 16, 1, 0],
        ],
        "delay_lights": .02,
        "skip_song": 0,
        "bpm": 120,
        "song_path": "songs/Radiohead - Everything in Its Right Place (Sam Goku Edit).ogg",
        "profiles": ["Shows"],
    },
}