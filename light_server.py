    ...
    while True:
        ...
        if 'title' in msg and 'original_bpm' in msg:
            ...
            if not handmade_song_found:
                ...
                fucked = ''.join(filter(lambda x: x in printable, rekordbox_title))
                ...
                print(f'Couldnt find handmade song "{rekordbox_title}", looking for an effect named "{rekordbox_effect_names[0]}"\n')
                rekordbox_effect_name = None
                for name in rekordbox_effect_names: # hack to fuzzy search kinda
                    if name in effects_config:
                        print_green(f'Found effect "{name}"\n')
                        rekordbox_effect_name = rekordbox_effect_name
                if not rekordbox_effect_name:
                    for effect in effects_config:
                        filtered = ''.join(filter(lambda x: x in printable, effect))
                        if filtered == fucked:
                            rekordbox_effect_name = effect
                    print_yellow(f'Cant play autogenerated light show effect from rekordbox! Missing effect {fucked}\n')
