...
GRID_ROW_LENGTH = 20
GRID_COL_LENGTH = 32
GRID_SIZE = GRID_ROW_LENGTH * GRID_COL_LENGTH * 3

GRID_ROW = np.arange(GRID_ROW_LENGTH)
GRID_COL = np.arange(GRID_COL_LENGTH)

grid = np.array(np.zeros((GRID_ROW_LENGTH, GRID_COL_LENGTH, 3)), np.double)
    ...
    while True:
        ...
        if not args.local:
            ...
            if grid_out == 0 and grid_in > 0:
                ...
                grid[:][:GRID_COL_LENGTH // 2] = [grid_levels[3], grid_levels[4], grid_levels[5]]
                grid[:][GRID_COL_LENGTH // 2:] = [grid_levels[0], grid_levels[1], grid_levels[2]]
